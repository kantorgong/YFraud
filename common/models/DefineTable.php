...
class DefineTable extends BaseActiveRecord
{
	...
	{
		return $this->getActions($this,'app\modules\cms');
	}
	...
	private static function getActionItem($table,$type,$actionId,$action)
	{
		...
		{
			return $type.'\actions\content\\'.$tableName.'\\'.$action;
		}
		else
		{
			return $type.'\actions\content\model_default\\'.$action;
		}
	}
	
	public static function getActions($table, $type='app\modules\cms')
	{
		$ret = [];
		...
		if(!empty($table[$type.'_action_custom']))
		{
			...
			if(!empty($customActions))
			{
				...
				{
					$ret[$id]=$type.'\actions\content\\'.$tableName.'\\'.$action;
				}
			}
		}
	...
	{
		return YGong::getAppParam('cachedTables');
	}
}
