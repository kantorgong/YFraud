...
use app\components\helper\TFileHelper;
...
// 		}
	...
	public static function getThemeUrl($resource = null)
	{
		...
		if($resource === null)
		{
			return YGong::getWebUrl() . '/static/themes/' . $currentTheme . '/';
		}
		return YGong::getWebUrl() . '/static/themes/' . $currentTheme . '/' . $resource;
	}
	...
	public static function getAttachUrl($url, $echo = true)
	{
		if($echo)
		{
			echo YGong::getWebUrl() . '/data/attachment/' . $url;
		}
		return YGong::getWebUrl() . '/data/attachment/' . $url;
	}

	public static function getCachedValue($cache, $id = null, $defaultValue = null)
	{
		$cacheItem = YGong::getAppParam($cache);
		if($cacheItem === null)
		{
	...
	{
		$cached = YGong::getAppParam('cachedConfigs');
		if($id!==null)
		{
			return $cached[$id];
		}
	...
	{
		$cached = YGong::getAppParam('cachedVariables');
		if($id!==null)
		{
			return $cached[$id];
		}
	...
	{
		$cached = YGong::getAppParam('cachedDicts');
	...
	public static function getDictsList($category,$pid)
	{
		$ret = [];
		$cached = YGong::getAppParam('cachedDicts');
		$dicts = $cached[$category];
